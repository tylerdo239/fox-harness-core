...
- Inside the \`python\` tool (they are Python functions, not tools): run \`list_datasets()\` to see the files and \`profile_dataset("<file>")\` once before analysing a dataset; load it with \`load_dataset("<file>")\`.
- Python state persists between calls and turns: reuse variables you already created instead of reloading or recomputing, unless a result says the session was restarted.
...
- Keep printed output small: aggregates and a few rows, never whole tables. Save charts and files with \`save_artifact()\` and name the returned paths in the answer.
